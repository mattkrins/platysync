...
export function rules(route) {
    ...
    route.post('/match', async (request, reply) => {
        ...
        try {
            ...
            if (!schema._rules[body.name])
                throw reply.code(409).send({ validation: { name: "Rule does not exist." } });
...
export default function rule(route) {
    route.post('/', form({
        ...
        try {
            ...
            const body = request.body;
            if (schema._rules[body.name])
                throw reply.code(409).send({ validation: { name: "Rule name taken." } });
            if (body.conditions.length <= 0)
                throw reply.code(400).send({ message: "No conditions defined." });
            const filePath = `${path}/schemas/${schema.name}/rules.yaml`;
    ...
    route.put('/:rule_name', form({
        ...
        const { schema_name, rule_name } = request.params;
        try {
            const schema = getSchema(schema_name, reply);
            const body = request.body;
            if (!schema._rules[rule_name])
                throw reply.code(404).send({ message: "Rule does not exist." });
    ...
    route.delete('/:rule_name', async (request, reply) => {
        ...
        try {
            const schema = getSchema(schema_name, reply);
            if (!schema._rules[rule_name])
                throw reply.code(404).send({ validation: { name: "Rule does not exist." } });
            const filePath = `${path}/schemas/${schema.name}/rules.yaml`;
            delete schema._rules[rule_name];
            schema.rules = schema.rules.filter(c => c.name !== rule_name);
            writeYAML(schema.rules, filePath);
    ...
    route.post('/:rule_name/copy', async (request, reply) => {
        ...
        try {
            ...
            if (!schema._rules[rule_name])
                throw reply.code(404).send({ message: "Rule does not exist." });
            if (schema._rules[newName])
                throw reply.code(409).send({ message: "Rule name taken." });
