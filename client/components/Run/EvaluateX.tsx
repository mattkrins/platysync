...
function find(query: string, r: primaryResult, caseSen: boolean){
...
function IconMap({ actions, size = 16, click }: { actions: actionResult[], size?: number, click?: (open: string)=> () => void }){
    ...
    return actions.map((action,key)=>{
        ...
        return <Tooltip key={key} fz="xs" withArrow label={action.display||action.name} color={col} >
...
interface Row { executed?: boolean, row: primaryResult, filters: {[k: string]: boolean}, check: (id: string) => () => void, view: (id: {name: string, open: string, actions: actionResult[]}) => void }
...
export default function Evaluate( { evaluated, setEvaluated, executed }: EvalProps ) {
    const { primary, initActions, finalActions, columns } = evaluated;
    ...
    const sort = (sorting: string) => {
        setSort(sorting);
        setEvaluated((e) => ({...e, primary: e.primary.sort((a, b) => {
            if (sorting==="display") return (a.display||a.id).localeCompare(b.display||b.id);
            return a.id.localeCompare(b.id);
        })}) );
    };
    const check = (id: string) => () => { setEvaluated((e) => ({...e, primary: e.primary.map(r=>r.id!==id?r:{...r, checked: !r.checked})}) ); };
    const checked = useMemo(()=>primary.filter(r=>r.checked), [ primary ]);
    const checkedCount = checked.length;
    const checkAll = () => setEvaluated((e) => ({ ...e,
        primary: e.primary.map(r=>{
    ...
    const cleaned = useMemo(()=> primary.filter(r=>{
        const errors = r.actions.filter(t=>t.result.error).length > 0;
        const warns = r.actions.filter(t=>t.result.warn).length > 0;
    ...
    <Box>
        ...
        {primary.length===0?<Notification icon={<IconQuestionMark size={20} />} withCloseButton={false} color="blue" title="None Found">No entries match the set conditions.</Notification>:
