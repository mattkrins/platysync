...
import { IconAlertCircle, IconBraces, IconChevronRight, IconCode, IconFiles, IconSearch } from "@tabler/icons-react";
...
export default function useTemplater( { allow }: { allow?: string[] } = {} ) {
    ...
    const templateProps = useCallback((form: UseFormReturnType<Record<string, unknown>>, path: string) => {
    ...
    filter(c=>allow?allow.includes(c.id):true).
    map(c=>({header: headers[c.name], provider: providers[c.id], ...c})),
    [ connectors, headers, providers, allow ]);
    
    const tags = useMemo(()=> flattenObjectToArray({...headers, $file: files.map((f: { name: string })=>f.name) }), [ headers, files ]);
