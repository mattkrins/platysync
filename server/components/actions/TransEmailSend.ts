...
export default async function TransEmailSend({ action, template, execute, data, configs, schema }: props<TransEmailSend>) {
    try {
        let emailer = action.config ? configs[action.config] as EMAIL : undefined;
        if (!emailer){
            const email = new EMAIL(schema, action, action.config);
            if (action.config) configs[action.config] = email;
        }
        
        //const x: EMAIL = action.config && configs[action.config] ? configs[action.config] as EMAIL : new EMAIL(action)
        //if (action.config){
