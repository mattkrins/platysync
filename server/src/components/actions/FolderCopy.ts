...
interface CopyFolder extends Action {
    source: string;
    target: string;
    validate: boolean;
    overwrite: boolean;
}
export default async function copyFolder(execute = false, act: Action, template: template): Promise <result> {
    const action = act as CopyFolder;
