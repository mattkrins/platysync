...
export default class StmcUpStuPassBulk extends Operation {
    connector!: string;
    source!: string;
    validate?: boolean;
    public async execute({ action, template, execute, data, connections, engine, ...rest }: props<this>) {
        ...
        try {
            ...
            const source = await this.open(data.source);
    ...
    private async open(path: string, encoding = 'utf8'): Promise<ParseResult<unknown>> {
