...
export default function Editor({ open, adding, close, refresh }: { open?: Connector, adding: boolean, close(): void, refresh(): void }) {
    return (
      <Modal size="xl" opened={!!open} title={adding?undefined:"Edit Connector"} onClose={close} styles={adding?{content:{background:"none"},header:{background:"none"}}:undefined} closeOnClickOutside={!adding} >
        {open&&<Content connector={open} refresh={refresh} adding={adding} close={close} />}
      </Modal>
    );
}